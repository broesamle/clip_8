...
function clip8directionOfPolyAngle(polyline, epsilon, minlen) {
    if (!(polyline instanceof SVGPolylineElement)) { throw "[clip8directionOfSVGLine] expected line element."; }
    if (!epsilon) { throw "[clip8directionOfSVGLine] expected epsilon to be a number > 0"; }
    if (!minlen) { throw "[clip8directionOfSVGLine] expected minlen to be a number > 0"; }
    var debug = true;
    if (debug) console.log("clip8directionOfPolyAngle: line, epsilon, minlen", polyline, epsilon, minlen);
    var coords = polyline.getAttribute("points").trim().split(/[\s,]+/);
    if (coords.length != 6) throw "[clip8directionOfSVGLine] expected 3 points (6 coordinates)."+coords.length+polyline.getAttribute("points").trim();
    ...
    if (absdeltax < epsilon && absdeltay > minlen)
        ...
        else throw "[clip8directionOfPolyAngle] Angle to flat (l/r). "+coords;
        ...
        else throw "[clip8directionOfPolyAngle] Angle to flat (u/d). "+coords;
    else throw "[clip8directionOfPolyAngle] Direction not detectable as left, right, up, down.";
}
